<?php
	...
	if(!empty($_POST)){
	     ...
	     $identification_carte = $_POST['identification_carte'];
		...
		if (isset($_POST['uploadBtn']) && $_POST['uploadBtn'] == 'Upload')
		{
		  if ((isset($_FILES['photo_recto'])) && (isset($_FILES['photo_verso'] )) && ($_FILES['photo_recto']['error'] === UPLOAD_ERR_OK) && ($_FILES['photo_verso']['error'] === UPLOAD_ERR_OK))
		  {
		    ...
		    if ((in_array($fileExtension, $allowedfileExtensions)) && (in_array($fileExtension, $allowedfileExtensions)))
		    {
  		...
  		else
  		{
  		  ...
  		  $message .= 'Error:' . $_FILES['photo_recto']['error'].' '. $_FILES['photo_vers']['error'];
		}
	     ...
	     $query = "SELECT * FROM clients WHERE numero_cni = '".$numero_cni."'";
             ...
             if($countrows>=3)
              {
                    echo "  <script type=\"text/javascript\">alert(\"Le client existe déjà\");
                     location =\"././form/index.php?id=66\"
                     </script>";
              }
              else{
	     	$query  = "insert into attente (id_promoteur,prenom_client,nom_client,date_naissance,lieu_naissance,date_delivrance,date_expiration,numero_cni,identification_carte,telephone,photo_recto,photo_verso) values ('".$id_promoteur."','".$prenom."', '".$nom."', '".$date_naissance."', '".$lieu_naissance."', '".$date_delivrance."','".$date_dexpiration."','".$numero_cni."','".$identification_carte."','".$telephone."','".$newFileName."','".$newFileName1."')";
	     	$query1  = "insert into clients(id_promoteur,prenom_client,nom_client,date_naissance,lieu_naissance,date_delivrance,date_expiration,numero_cni,identification_carte,telephone,photo_recto,photo_verso) values ('".$id_promoteur."','".$prenom."', '".$nom."', '".$date_naissance."', '".$lieu_naissance."', '".$date_delivrance."','".$date_dexpiration."','".$numero_cni."','".$identification_carte."','".$telephone."','".$newFileName."','".$newFileName1."')";
		...
		header('Location: ./form/index.php?id=23');
	}
?>
